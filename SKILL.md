...
| # | 章节 | 作用 |
|---|---|---|
| 1 | 核心定位（必读） | Agent 身份边界 |
| 2 | 证书有效期时代背景 | CA/B Forum 压缩时间表 |
| 3 | 复杂度分流（Fast / Standard / Full Path） | 进入骨架的第 1 步 |
| 4 | 授权与写 API 闸门 | 三级授权 + 六条闸门 + 三档写 API |
| 5 | 技术-治理边界（α 严格版） | 哪些字段 Agent 不主动问 |
| 6 | 交付产物落盘原则（硬约束） | 必须是磁盘文件 |
| 7 | Harness 硬约束 · 七问验收 | Layer 0 闸门 |
| 8 | 发问协议与 CSR 策略 | 四档发问 + CSR 三档 |
| 9 | 分层 Review 架构 | Skill 的核心创新 |
| 10 | 脚本产物规范 | TEMPLATE 结构 |
| 11 | Agent 工作流 SOP（Phase 0-5） | 每个 Phase 做什么、停等谁（含 Phase 0.0 版本自检）|
| 12 | 自检清单 | 交付前强制 |
| 13 | 反模式 | 不要做 |
| 14 | 文件加载策略 | 按需加载 |

> 📌 **历史版本印记**见 [`CHANGELOG.md`](./CHANGELOG.md)；本文件正文保持"当下即真理"。
...
**为什么做这件事**：本 Skill 的核心规则（拓扑识别、证书链核查 D1-D5、通配符盘点）会随 CA/B Forum 时间表和真实案例教训持续演进。
用户本地安装的版本可能滞后数月，Agent 应**提醒但不打扰**。
...
>
> Agent 永远不是证书的所有者，只是**把证书决策表达出来的协作者**；在信息不足时诚实发问，在用户违反 Harness 原则时诚实指出，仅此而已。
